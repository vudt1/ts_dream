...
- [x] **Login skill purge (§5.6 / §5.4 note 2)**: cuối `login_db` chạy `DELETE FROM skill WHERE player_id = ? AND Id >= 0 AND Id <= 9` — đúng `Logined1()` tail C# `Client.cs:8193`, sau khi stats frame đã build (nên Logined1 skill list vẫn là pre-purge như C#). Repository fn `db::persist::delete_system_skills` (no-op khi `pool=None`).
