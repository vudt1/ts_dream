...
| SubOp | Hành động |
| :--- | :--- |
| `1` | **Mở form popup theo id**: `func_0x0055374c(mgr, RP[2], 0)` (hàm nằm trong HOLE — xem §4.1). Nếu `RP[2] == 3` thì đọc thêm `RP[3]`: `1` → ghi `form#3 + 0x38 = 100`; `2` → `= 1000` (value tùy chọn, xem §5.3). |
...
- Chuỗi banner `0x799200/0x799224` **chưa dump** — mock server không cần biết nội dung (client tự in literal cục bộ), nhưng tài liệu này không dịch được (§7).
...
case SubOp of
  1: begin
       TSportManage_OpenForm(RP[2], 0);    // func_0x0055374c — HOLE (§4.1)
       if RP[2] = 3 then
...
| `[2]` (SubOp 1) | `id` form muốn mở | hàm mở nằm trong HOLE; id hợp lệ suy ra từ bảng đóng: `{1,2,3,4,6,0xFF}` (§5.2) |
...
- `index.csv` (kiểm tra bằng python): không có hàm nào phủ `0x0055374C`; `FUN_00553410` kết thúc tại `0x005534E3`, tiếp đó là `TSportManage.Create` `0x005534E4–0x00553525`, `FUN_00553528` `0x00553528–0x0055355D`, rồi `FUN_00553818` tại `0x00553818` → **hole `0x0055355D–0x00553818` chưa `0x0055374C`**.
- Bằng chứng mở rộng quanh hole (không phải thân hàm):
  - ngay trong hole có `0x00553574 → FUN_00553410 [UNCONDITIONAL_CALL]` (`00553410.c:14`) — code hole gọi lại đóng form;
  - `0x0055374C` được gọi **DUY NHẤT** tại đây (`0078a89c.c:6876` + `case_050.c:39`), ký hiệu `func_0x...` của Ghidra (hàm trong hole, không export).
- **Suy luận có kiểm chứng (không khẳng định)**: đây là `OpenForm(id, flag)` của `TSportManage` — bằng chứng gián tiếp: (a) `FUN_00553410` map `mgr+4 ∈ {1,2,3,4,6,0xFF}` → form tương ứng; (b) SubOp 1 xử lý riêng `id==3` vì referencing `gvar_007DA778` (đúng là form map với id 3 trong `00553410.c:39–41`); (c) nhiều guard input chỉ cho thao tác khi `*(mgr+4)==0` (§5.1) tức "chưa mở form nào". **Chi tiết nhánh của id 1/2/4/6/0xFF và giá trị mode của form 3 ⇒ UNKNOWN cho đến khi redump `0x0055355D–0x00553818`.**
...
**Thân `FUN_007BADB0`** (asm `007badb0_FUN_007badb0.asm.txt:4–25`): lưu `EAX=self, DL, ECX`; `PUSH [EBP+8]` (arg stack đầu = con trỏ chuỗi); gọi `FUN_007AFBF8` rồi `FUN_007AFEF8`; `FUN_007AFEF8` (`007afef8.c:66–76`) duyệt chuỗi node `node := node[+8]` đến node cuối (tail of list), kiểm tra byte `node+0x14` — nếu busy thì `RET 0x4` (pop 1 arg) **bỏ qua thông báo**; đuôi `0x7AFC1F–0x7AFC24` của `FUN_007AFBF8` **không có trong asm dump** (cắt cụt) → phần enqueue/format thực tế nằm ngoài vùng export.
...
**Literal** `&UNK_00799200` / `&UNK_00799224` (cách nhau 0x24): xem §7.
...
- **`mgr+4` = ActiveId** — popup đang mở (bảng id→form §4.2). Writer của `+4`: KHÔNG có trong SSOT export (chỉ có reader) → thuộc về `func_0x0055374c` trong HOLE. Người đọc `+4` (hữu ích cho mock — chứng minh "1 form/lần"): guard chuột/phím `005186e4.c:68`, `0050bff8.c:135`, `00566884.c:27`, `005eee60.c:60`, `00611330.c:30`, `00717e78.c:165`, `0073ce00.c:35`, `007c511c.c:29`, và `00777448.c:34`.
...
### 5.2. Bảng popup id→form (người dùng)

| id | Global | Tick/refresh (nhánh id của `FUN_00553840`/`0055391c`) | Ghi chú khác |
| :--- | :--- | :--- | :--- |
| 1 | `gvar_007DA42C` | `FUN_00548080` (`00548080.c`, máy trạng thái animation `+0x61`) | Case `0x3A` dispatcher `:6932` bơm payload vào `FUN_00547C84`; **gọi xin gửi 0x39** §6 |
| 2 | `gvar_007DA0F4` | `0x5524c4` / `0x552e58` | `00553528.c:21–23`: double-click → `Form2.Close(vmt+4)`; **chính là object của OP 0x3B** (`opcode_3b.md`) |
| 3 | `gvar_007DA778` | `0x541c6c` / `0x541f8c` | **form mục tiêu của SubOp 1 mode==3**; chi tiết dưới |
...
- 16 reference duy nhất trong SSOT: free/reset (`00553410.c:40–41`), 2 tick (§5.2), **2 write của chính OP 0x39** (`case_050.c:54,57`), và 4 method `func_0x00541098/0x005408F4/0x0053FD78/0x0053F8FC` được **OP 0x3C** bơm payload (dispatcher `:6972–6984`, `case_053_0079575C.c:29–38`) — **cả 4 đều trong HOLE** (kiểm tra `index.csv`).
- `+0x38` (int): **chỉ được ghi 100/1000 bởi OP 0x39 SubOp 1; KHÔNG có bất kỳ read/export nào** (quét `+ 0x38)` trong khoảng địa chỉ 0x53E000–0x542800 = 0 kết quả). Nghi vấn "giá trị cửa/cược" cho 2 chế độ của form quay số — **UNKNOWN, không khẳng định**.
...
- `gvar_007D9D30` = **TFConnect** — kênh game-server thứ hai theo handoff (`handoff-2026-09-10-protocol-analysis.md`, mục Wire format); được truyền làm `param_1` của `FUN_0077F414` ở §6 và được Form id1 "tự-inject" message `DL=0x1A` vào dispatcher (`00548080.c:154–160`).
- `gvar_007DA5A0` = object dữ liệu lớn (mảng `+0x9a10+i*4`, flag `+0xa096`) — `FUN_00553818` set flag `=1` (§6); sinh trong HOLE.
...
- **`0x00799200` và `0x00799224`: CHƯA DUMP — không dịch được.** `redump/` chỉ có `lit_5957D8/595800/595810/595828/595844/596078`, `lit_77F771`, `lit_78A854`, `lit_7A2094/7A20A8`, `lit_7ABD54/BDAC/BDF0/BE40/BE64/BE74` — không file nào phủ 0x799xxx.
- Khu vực này là **block chuỗi của họ banner**: OP 0x37 cũng gọi `+0x90` với `&UNK_007991d8`, `&UNK_007991ec` (2000ms) (`0078a89c.c:6840,6844`); khoảng cách 0x20–0x24/entry. **Đề nghị redump**: từ `0x007991C0` đến hết `0x00799248` (cắt tới null-terminator từng entry) rồi giải theo tiền lệ **cp1258 → NFC** (`opcode_02.md §5`); nếu ra vẫn mơ hồ, thử **VISCII** (ngoại lệ `opcode_13.md`) và ghi bằng chứng. KHÔNG bịa nội dung.
...
9. `index.csv` (python): HOLE `0x0055355D–0x00553818` (chứa `func_0x0055374c`), method form id3 `0x0053F/0x00540/0x00541`, caller tick `0x515C15/0x515D8E`; `redump/` không phủ `0x799xxx`; `00516158.c:86–93` — RP pipeline (tiền lệ); `0050cd6c.c:67` + `005163e4.asm.txt:53–55` + `token_recv/send.hex` — XOR phủ toàn frame.

**UNKNOWN (còn lại):**
- Thân `func_0x0055374c` = `OpenForm`: nhánh id 1/2/4/6/0xFF, hành xử vs id trùng/lặp, nghi thức đặt `mgr+4` — **redump `0x0055355D–0x00553818`**.
- Nội dung 2 banner `0x799200/0x799224` (và 0x7991d8/0x7991ec của OP 0x37) — **redump `0x7991C0–0x799248`**, thử cp1258→NFC trước, VISCII làm phương án hai.
- Consumer của form3`+0x38` (100/1000): không có trong export (nghi vấn nằm trong 4 method id3 thuộc OP 0x3C đang HOLE).
- Đuôi `0x7AFC1F–0x7AFC24` của `FUN_007AFBF8` (phần enqueue chuỗi/thời gian thật của slot +0x90) — asm dump cắt cụt.
- Khai sinh `gvar_007DA778/D9D30/DA5A0` (lazy trong HOLE); ý nghĩa flag `**(gvar_007DA5A0)+0xa096`; giá trị `CL=1` trong dead-send (sub-op ?) — không kết luận.
