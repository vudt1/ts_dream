...
| Mã bản đồ/khu vực | Word tại offset đối tượng `+0x63a` → quyết định blend/animation qua `FUN_0070d86c`, kiểm tra vùng đặc biệt qua `FUN_00634f3c`/`func_0x0075c184`/`func_0x0054c29c`/`func_0x0054a384` |
...
3. `gself+0x1437 = FUN_00634f3c(itemDB0x98BE0E_tbl, MapID, 3)`; các cờ phái sinh `+0x1458/+0x1459/+0x145c = func_0x0075c184 / func_0x0054c29c / func_0x0054a384 (MapID)` → *nhãn "kiểu bản đồ/scene đặc biệt"*.
...
10. **Kết thúc nghiệp vụ:** `FUN_0071e2b8(gself, posX, posY)` đặt tọa độ thế giới; `FUN_0070d86c(gself)` chọn chế độ blend/anim theo MapID *(đồ họa)*; `func_0x00509e84(gvar_007DA37C)`; `FUN_00778510(gvar_007D9C28, posX, posY)` hiệu chỉnh camera/frame theo vị trí *(đồ họa)*; `FUN_0071e080(gself, +0x4b0, +0x4b1)` bật/tắt đối tượng "xe đẩy/NPC đồng hành" `TCartNpc` *(1 dòng)*; `FUN_0071fae0(gself, posX, posY)` tính lại neo render/animation theo camera *(đồ họa)*. Nếu `gself+0x378 == 0` thì gọi `FUN_005c2420`/`func_0x005c2658(0x20)`/`func_0x005c280c(3)` (đặt mode hiệu ứng).
...
6. `idx = func_0x0070c158(gvar_007D9D34, charID)` — lấy/chỉ định chỉ số actor (1..800). Nếu `idx != 0`:
   - nếu slot `gvar_007DA300[idx]` trống → `TPlayers_Create(VMT_70B5D0, 1, idx)` gắn vào mảng actor.
   - tăng số actor: `*(gvar_007D9D34 + 0x5c)++`.
...
```
- Đây là một **lệnh gửi tối giản dạng `[0x03][1 byte]`** (byte đầu = chính `param_2 = 0x03`), **không đối xứng 1-1** với gói S→C OP 0x03 (vốn là broadcast dung lượng lớn do **server chủ động đẩy**, client không "yêu cầu" nó theo kiểu request/response).
- **Không tìm thấy call-site tường minh** gọi `SendCommand(3)` với hằng số `3` trong toàn bộ `ts_decompile` (các call-site dùng opcode dạng `CONCAT31(...,0x3a..0x3c)` là các OP 0x3A–0x3C khác). Vì vậy `case 3` C→S nhiều khả năng là **lệnh điều khiển nội bộ/legacy**, không phục vụ trực tiếp luồng spawn. Đánh dấu **độ tin cậy thấp**, cần kiểm chứng thêm động (hook socket) trước khi đưa vào đặc tả.
- Kết luận cho Mock Server: **chiều gửi `0x03` không bắt buộc** để dựng luồng vào cảnh; chỉ cần server **phát** gói S→C `0x03` là client tự spawn.
...
- **Hàm tiện ích:** `00745bc0_FUN_00745bc0.c` (giải mã ngoại hình), `0071e2b8_FUN_0071e2b8.c` (đặt tọa độ), `0070c20c_FUN_0070c20c.c` & `func_0x0070c158` (tìm/chỉ định slot actor 1..800), `004c9bf0_FUN_004c9bf0.c` (phân nhóm ID/map), `00504c9c_FUN_00504c9c.c` (điều kiện scene login %100∈[90..99]), `00774af8_FUN_00774af8.c` (ô trang bị), `0077499c_FUN_0077499c.c` (sprite item), `00634f3c_FUN_00634f3c.c` (danh sách model/look đặc biệt).
...
**Hằng số đã định vị:** `DAT_00796EB4` = chuỗi tiền tố tên (chỉ hiển thị ở scene login) — *chưa giải được nội dung vì không có dump `.strings` tại `0x00796EB4` trong `ts_decompile/redump/`; cần redump vùng dữ liệu này để xác minh nội dung chính xác.*
