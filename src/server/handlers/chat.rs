...
use crate::server::handler::{OpcodeCtx, HandleOutcome};
...
/// Admin id threshold (C# `Client.isAdmin()`: `_My_Id < 300012`).
...
pub async fn handle_chat(ctx: &mut OpcodeCtx<'_>) {
    ...
    match sub {
        ...
        2 => {
            ...
            if text.starts_with('/') {
                let msg = text.trim();
                handle_slash(conn, out, pool, msg).await;
                return;
            }
            // Channel selection (C# `Toan`/`Gan`): global item → op 0x02 sub
            // 0x01 to every client; otherwise map chat → sub 0x02.
            let frame = if wears_global_chat_item(conn) {
                spawn::chat_frame(1, conn.session.id, payload)
            } else {
                spawn::chat_frame(2, conn.session.id, payload)
            };
            out.send(&frame);
            if let Some(hub) = hub {
...
/// Dispatch a slash command (C# `FChat.H2` slash chain).
async fn handle_slash(
    ...
    let (cmd, args) = split_slash(msg);
    let admin = is_admin(conn.session.id);
    let player_id = conn.session.id;

    // --- Admin-only commands (C# `isAdmin()` block) ---
    if admin {
        match cmd.as_str() {
            "/additem" => {
                if args.is_empty() {
            ...
            "/test" => {
                ...
                // id, count, texp32, 05, Jam-level, 03, texp32, 00
    ...
    match cmd.as_str() {
        ...
        "/openhotel" => {
            // C# `OpenHotel`: one `1F06` frame per stable pet (stt 5..8) + 1F07.
            let mut entries = String::new();
            for p in conn.session.pets.iter().filter(|p| (5..=8).contains(&p.stt)) {
            ...
            if !entries.is_empty() {
                out.send(crate::protocol::frame("1F06", &entries));
            }
        ...
        "/openbank" => {
            // C# `OpenBank`: bank screen (TienTrang money shown as 0 here).
            out.send("F44403001D0900".to_string());
            out.send(format!("F44406001D04{}", encoder::le32(0)));
...
mod tests {
    ...
    async fn map_chat_echoes_self() {
        ...
        let mut ctx = crate::server::handler::test_ctx(&mut conn, &data, &service, &mut out, 2, payload);
    ...
    async fn global_chat_item_selects_0201() {
        ...
        let data = GameData::default();
        let service = BattleService::new(Arc::new(GameData::default()));
        let mut ctx = crate::server::handler::test_ctx(&mut conn, &data, &service, &mut out, 2, b"HI");
        handle_chat(&mut ctx).await;
    ...
    async fn slash_where_returns_sysmsg() {
        ...
        let data = GameData::default();
        let service = BattleService::new(Arc::new(GameData::default()));
        let mut ctx = crate::server::handler::test_ctx(&mut conn, &data, &service, &mut out, 2, b"/where");
        handle_chat(&mut ctx).await;
        assert_eq!(out.outgoing.len(), 1);
    ...
    async fn slash_additem_requires_admin() {
        ...
        let data = GameData::default();
        let service = BattleService::new(Arc::new(GameData::default()));
        let mut ctx = crate::server::handler::test_ctx(&mut conn, &data, &service, &mut out, 2, b"/additem 10001,2");
        handle_chat(&mut ctx).await;
        assert!(
    ...
    async fn slash_battle_sets_trigger() {
        ...
        let data = GameData::default();
        let service = BattleService::new(Arc::new(GameData::default()));
        let mut ctx = crate::server::handler::test_ctx(&mut conn, &data, &service, &mut out, 2, b"/battle 5");
        handle_chat(&mut ctx).await;
    ...
    async fn whisper_builds_frame_with_recipient_id() {
        ...
        let data = GameData::default();
        let service = BattleService::new(Arc::new(GameData::default()));
        let mut ctx = crate::server::handler::test_ctx(&mut conn, &data, &service, &mut out, 3, &payload);
        handle_chat(&mut ctx).await;
        assert_eq!(out.outgoing.len(), 1);
    ...
    async fn party_chat_frame_uses_0205() {
        ...
        let data = GameData::default();
        let service = BattleService::new(Arc::new(GameData::default()));
        let mut ctx = crate::server::handler::test_ctx(&mut conn, &data, &service, &mut out, 5, b"hello party");
        handle_chat(&mut ctx).await;
        assert_eq!(out.outgoing.len(), 1);
