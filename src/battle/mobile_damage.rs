...
mod tests {
    ...
    #[test]
    fn mobile_element_cycle_and_neutral_rules() {
        assert_eq!(element_multiplier(1, 2), 1.5);
        assert_eq!(element_multiplier(1, 4), 0.75);
        assert_eq!(element_multiplier(5, 1), 1.0);
        assert_eq!(element_multiplier(7, 8), 1.5);
    }

    #[test]
    fn mobile_base_damage_matches_kotlin_integer_branches() {
        let a = fighter();
        assert_eq!(
    ...
    #[test]
    fn mobile_attack_is_reproducible_for_seed() {
        let a = fighter();
        let mut t = fighter();
        t.typ = 3;
        t.thuoctinh = 2;
        let mut r1 = DotNetRandom::new(123);
        let mut r2 = DotNetRandom::new(123);
        let x = calculate_attack(
            &mut r1,
            &a,
            &t,
            Some(MobileSkillInput {
                element: 1,
                numerical: 100,
                attribute: 28,
                round: 0,
